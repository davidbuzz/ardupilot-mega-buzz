...
void QuantonRCOutput::init(void* unused) 
{
  ...
  //  _alt_fd = -1;    
    ...
    /* if we have 8 servos, then we are attached to QuantonIO. In that
     * case, we want to open the QuantonFMU driver for the 4 additional
     * channels
     */
    if (_servo_count <= 4) {
...
void QuantonRCOutput::set_freq(uint32_t chmask, uint16_t freq_hz) 
{
...
uint16_t QuantonRCOutput::get_freq(uint8_t ch) 
{
    if (_rate_mask & (1<<ch)) {
        return _freq_hz;
    }
...
void QuantonRCOutput::enable_ch(uint8_t ch)
{
    // channels are always enabled ...
}

void QuantonRCOutput::enable_mask(uint32_t chmask)
{
    // channels are always enabled ...
}

void QuantonRCOutput::disable_ch(uint8_t ch)
{
    // channels are always enabled ...
}

void QuantonRCOutput::disable_mask(uint32_t chmask)
{
    // channels are always enabled ...
}

void QuantonRCOutput::write(uint8_t ch, uint16_t period_us)
{
...
void QuantonRCOutput::write(uint8_t ch, uint16_t* period_us, uint8_t len)
{
...
uint16_t QuantonRCOutput::read(uint8_t ch) 
{
    if (ch >= Quanton_NUM_OUTPUT_CHANNELS) {
        return 0;
    }
    return _period[ch];
}

void QuantonRCOutput::read(uint16_t* period_us, uint8_t len)
{
    for (uint8_t i=0; i<len; i++) {
        period_us[i] = read(i);
    }
...
void QuantonRCOutput::_timer_tick(void)
{
