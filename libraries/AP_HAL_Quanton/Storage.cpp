...
void QuantonStorage::_storage_create(void)
{
	...
	for (uint16_t loc=0; loc<sizeof(_buffer); loc += Quanton_STORAGE_MAX_WRITE) {
		if (write(fd, &_buffer[loc], Quanton_STORAGE_MAX_WRITE) != Quanton_STORAGE_MAX_WRITE) {
...
void QuantonStorage::_storage_open(void)
{
	if (_initialised) {
		return;
	}
 ...
 */
void QuantonStorage::_mark_dirty(uint16_t loc, uint16_t length)
{
	uint16_t end = loc + length;
	while (loc < end) {
		uint8_t line = (loc >> Quanton_STORAGE_LINE_SHIFT);
		_dirty_mask |= 1 << line;
		loc += Quanton_STORAGE_LINE_SIZE;
	}
}

uint8_t QuantonStorage::read_byte(uint16_t loc) 
{
...
uint16_t QuantonStorage::read_word(uint16_t loc) 
{
	uint16_t value;
...
uint32_t QuantonStorage::read_dword(uint16_t loc) 
{
	uint32_t value;
	if (loc >= sizeof(_buffer)-(sizeof(value)-1)) {
		return 0;
...
void QuantonStorage::read_block(void *dst, uint16_t loc, size_t n) 
{
...
void QuantonStorage::write_byte(uint16_t loc, uint8_t value) 
{
	if (loc >= sizeof(_buffer)) {
		return;
	}
...
void QuantonStorage::write_word(uint16_t loc, uint16_t value) 
{
	if (loc >= sizeof(_buffer)-(sizeof(value)-1)) {
		return;
	}
...
void QuantonStorage::write_dword(uint16_t loc, uint32_t value) 
{
	if (loc >= sizeof(_buffer)-(sizeof(value)-1)) {
		return;
	}
...
void QuantonStorage::write_block(uint16_t loc, const void *src, size_t n) 
{
	if (loc >= sizeof(_buffer)-(n-1)) {
		return;
	}
...
void QuantonStorage::_timer_tick(void)
{
	...
	uint8_t i, n;
	for (i=0; i<Quanton_STORAGE_NUM_LINES; i++) {
		if (_dirty_mask & (1<<i)) {
			break;
		}
	}
	if (i == Quanton_STORAGE_NUM_LINES) {
	...
	for (n=1; (i+n) < Quanton_STORAGE_NUM_LINES && 
		     n < (Quanton_STORAGE_MAX_WRITE>>Quanton_STORAGE_LINE_SHIFT); n++) {
	 ...
	 */
	if (lseek(_fd, i<<Quanton_STORAGE_LINE_SHIFT, SEEK_SET) == (i<<Quanton_STORAGE_LINE_SHIFT)) {
		_dirty_mask &= ~write_mask;
		if (write(_fd, &_buffer[i<<Quanton_STORAGE_LINE_SHIFT], n<<Quanton_STORAGE_LINE_SHIFT) != n<<Quanton_STORAGE_LINE_SHIFT) {
