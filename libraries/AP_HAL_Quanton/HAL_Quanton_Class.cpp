...
static QuantonScheduler schedulerInstance;
static QuantonStorage storageDriver;
static QuantonRCInput rcinDriver;
static QuantonRCOutput rcoutDriver;
static QuantonAnalogIn analogIn;
...
bool _Quanton_thread_should_exit = false;        /**< Daemon exit flag */
...
void HAL_Quanton::init(int argc, char * const argv[]) const 
{
    int i;
    ...
    for (i=0; i<argc; i++) {
        ...
        if (strcmp(argv[i], "status") == 0) {
            if (_Quanton_thread_should_exit && thread_running) {
                printf("\t%s is exiting\n", SKETCHNAME);
...
const HAL_Quanton AP_HAL_Quanton;
