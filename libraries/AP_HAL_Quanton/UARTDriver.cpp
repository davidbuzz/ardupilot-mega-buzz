...
QuantonUARTDriver::QuantonUARTDriver(const char *devpath, const char *perf_name) :
	_devpath(devpath),
    _fd(-1),
    _baudrate(57600),
...
void QuantonUARTDriver::begin(uint32_t b, uint16_t rxS, uint16_t txS) 
{
    ...
    // on Quanton we have enough memory to have a larger transmit and
...
void QuantonUARTDriver::begin(uint32_t b) 
{
	begin(b, 0, 0);
}
 ...
 */
void QuantonUARTDriver::try_initialise(void)
{
    if (_initialised) {
        return;
    }
...
void QuantonUARTDriver::flush() {}

bool QuantonUARTDriver::is_initialized() 
{ 
    try_initialise();
    return _initialised; 
}

void QuantonUARTDriver::set_blocking_writes(bool blocking) 
{
    _nonblocking_writes = !blocking;
}

bool QuantonUARTDriver::tx_pending() { return false; }
 ...
 */
int16_t QuantonUARTDriver::available() 
{ 
	if (!_initialised) {
        try_initialise();
		return 0;
 ...
 */
int16_t QuantonUARTDriver::txspace() 
{ 
	if (!_initialised) {
        try_initialise();
		return 0;
 ...
 */
int16_t QuantonUARTDriver::read() 
{ 
	uint8_t c;
    if (!_initialised) {
        try_initialise();
 ...
 */
size_t QuantonUARTDriver::write(uint8_t c) 
{ 
    if (!_initialised) {
        try_initialise();
        return 0;
 ...
 */
size_t QuantonUARTDriver::write(const uint8_t *buffer, size_t size)
{
	if (!_initialised) {
        try_initialise();
		return 0;
 ...
 */
int QuantonUARTDriver::_write_fd(const uint8_t *buf, uint16_t n)
{
    int ret = 0;
 ...
 */
int QuantonUARTDriver::_read_fd(uint8_t *buf, uint16_t n)
{
    int ret = 0;
 ...
 */
void QuantonUARTDriver::_timer_tick(void)
{
    uint16_t n;
