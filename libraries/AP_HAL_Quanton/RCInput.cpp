...
void QuantonRCInput::init(void* unused)
{
...
uint8_t QuantonRCInput::read(uint16_t* periods, uint8_t len) 
{
...
bool QuantonRCInput::set_overrides(int16_t *overrides, uint8_t len) 
{
	bool res = false;
	for (uint8_t i = 0; i < len; i++) {
...
bool QuantonRCInput::set_override(uint8_t channel, int16_t override) {
	if (override < 0) {
...
void QuantonRCInput::clear_overrides()
{
...
void QuantonRCInput::_timer_tick(void)
{
