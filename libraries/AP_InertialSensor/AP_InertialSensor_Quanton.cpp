 ...
 */
void AP_InertialSensor_Quanton::_set_filter_frequency(uint8_t filter_hz)
{
    if (filter_hz == 0) {
...
bool AP_InertialSensor_Quanton::get_gyro_health(uint8_t instance) const
{
...
uint8_t AP_InertialSensor_Quanton::get_gyro_count(void) const
{
    return _num_gyro_instances;
}

bool AP_InertialSensor_Quanton::get_accel_health(uint8_t k) const
{
    if (_sample_time_usec == 0 || _last_get_sample_timestamp == 0) {
        // not initialised yet, show as healthy to prevent scary GCS
        // warnings
...
uint8_t AP_InertialSensor_Quanton::get_accel_count(void) const
{
    return _num_accel_instances;
}

bool AP_InertialSensor_Quanton::update(void) 
{
...
float AP_InertialSensor_Quanton::get_gyro_drift_rate(void) 
{
    // assume 0.5 degrees/second/minute
    return ToRad(0.5/60);
}

void AP_InertialSensor_Quanton::_get_sample(void)
{
    for (uint8_t i=0; i<_num_accel_instances; i++) {
...
bool AP_InertialSensor_Quanton::_sample_available(void)
{
...
bool AP_InertialSensor_Quanton::wait_for_sample(uint16_t timeout_ms)
{
 ...
 */
bool AP_InertialSensor_Quanton::healthy(void) const
{
    return get_gyro_health(0) && get_accel_health(0);
}

uint8_t AP_InertialSensor_Quanton::_get_primary_gyro(void) const 
{
    for (uint8_t i=0; i<_num_gyro_instances; i++) {
