/*
 *       APM_RC_PX4.cpp - Radio Control Library for Ardupilot Mega. Arduino
...
****************************************************/
void APM_RC_PX4::_timer4_capt_cb(void)
{
...
APM_RC_PX4::APM_RC_PX4()
{
}

// Public Methods //////////////////////////////////////////////////////////////
void APM_RC_PX4::Init( Arduino_Mega_ISR_Registry * isr_reg )
{
...
uint16_t APM_RC_PX4::OutputCh_current(uint8_t ch)
{
    uint16_t pwm=0;
    switch(ch) {
    	/* 
...
void APM_RC_PX4::enable_out(uint8_t ch)
{
    switch(ch) {
    	/* 
...
void APM_RC_PX4::disable_out(uint8_t ch)
{
    switch(ch) {
    	/* 
    case 0:  TCCR5A &= ~(1<<COM5B1); break;  // CH_1 : OC5B
...
uint16_t APM_RC_PX4::InputCh(uint8_t ch)
{
    uint16_t result;
...
uint8_t APM_RC_PX4::GetState(void)
{
...
void APM_RC_PX4::Force_Out(void)
{
    Force_Out0_Out1();
    Force_Out2_Out3();
    Force_Out6_Out7();
...
void APM_RC_PX4::Force_Out0_Out1(void)
{
...
void APM_RC_PX4::SetFastOutputChannels(uint32_t chmask, uint16_t speed_hz)
{
...
bool APM_RC_PX4::setHIL(int16_t v[NUM_CHANNELS])
{
    uint8_t sum = 0;
...
void APM_RC_PX4::clearOverride(void)
{
    for (uint8_t i=0; i<NUM_CHANNELS; i++) {
        _HIL_override[i] = 0;
    }
