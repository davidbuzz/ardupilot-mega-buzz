...
setup_mode(uint8_t argc, const Menu::arg *argv)
{
    ...
    hal.uart0->printf_P(PSTR("Setup Mode\n"
                         "\n"
...
setup_show(uint8_t argc, const Menu::arg *argv)
{
    ...
    hal.uart0->printf_P(PSTR("Raw Values\n"));
    print_divider();
...
{
    //print_blanks(2);
    hal.uart0->printf_P(PSTR("Batt Mointor\n"));
    print_divider();
    if(g.battery_monitoring == 0) hal.uart0->printf_P(PSTR("Batt monitoring disabled"));
    if(g.battery_monitoring == 3) hal.uart0->printf_P(PSTR("Monitoring batt volts"));
    if(g.battery_monitoring == 4) hal.uart0->printf_P(PSTR("Monitoring volts and current"));
    print_blanks(2);
}
static void report_radio()
{
    //print_blanks(2);
    hal.uart0->printf_P(PSTR("Radio\n"));
    print_divider();
    // radio
    print_radio_values();
    print_blanks(2);
...
{
    //print_blanks(2);
    hal.uart0->printf_P(PSTR("Gains\n"));
    print_divider();

#if APM_CONTROL == DISABLED
	hal.uart0->printf_P(PSTR("servo roll:\n"));
	print_PID(&g.pidServoRoll);

    hal.uart0->printf_P(PSTR("servo pitch:\n"));
    print_PID(&g.pidServoPitch);

	hal.uart0->printf_P(PSTR("servo rudder:\n"));
	print_PID(&g.pidServoRudder);
#endif

    hal.uart0->printf_P(PSTR("nav roll:\n"));
    print_PID(&g.pidNavRoll);

    hal.uart0->printf_P(PSTR("nav pitch airspeed:\n"));
    print_PID(&g.pidNavPitchAirspeed);

    hal.uart0->printf_P(PSTR("energry throttle:\n"));
    print_PID(&g.pidTeThrottle);

    hal.uart0->printf_P(PSTR("nav pitch alt:\n"));
...
{
    //print_blanks(2);
    hal.uart0->printf_P(PSTR("Crosstrack\n"));
    print_divider();
    // radio
    hal.uart0->printf_P(PSTR("XTRACK: %4.2f\n"
                         "XTRACK angle: %d\n"),
...
{
    //print_blanks(2);
    hal.uart0->printf_P(PSTR("Throttle\n"));
    print_divider();

    hal.uart0->printf_P(PSTR("min: %d\n"
                         "max: %d\n"
                         "cruise: %d\n"
                         "failsafe_enabled: %d\n"
                         "failsafe_value: %d\n"),
...
static void report_compass()
{
    ...
    case AP_COMPASS_TYPE_HMC5883L:
        hal.uart0->println_P(PSTR("HMC5883L"));
        break;
    case AP_COMPASS_TYPE_HMC5843:
        hal.uart0->println_P(PSTR("HMC5843"));
        break;
    case AP_COMPASS_TYPE_HIL:
        hal.uart0->println_P(PSTR("HIL"));
        break;
    default:
    ...
    // mag declination
    hal.uart0->printf_P(PSTR("Mag Declination: %4.4f\n"),
                    degrees(compass.get_declination()));

    Vector3f offsets = compass.get_offsets();

    // mag offsets
    hal.uart0->printf_P(PSTR("Mag offsets: %4.4f, %4.4f, %4.4f\n"),
                    offsets.x,
                    offsets.y,
                    offsets.z);
    print_blanks(2);
...
{
    //print_blanks(2);
    hal.uart0->printf_P(PSTR("Flight modes\n"));
    print_divider();
...
print_PID(PID * pid)
{
    hal.uart0->printf_P(PSTR("P: %4.3f, I:%4.3f, D:%4.3f, IMAX:%ld\n"),
                    pid->kP(),
                    pid->kI(),
                    pid->kD(),
                    (long)pid->imax());
...
{
    if(b)
        hal.uart0->printf_P(PSTR("en"));
    else
        hal.uart0->printf_P(PSTR("dis"));
    hal.uart0->printf_P(PSTR("abled\n"));
}

static void
print_accel_offsets(void)
{
    hal.uart0->printf_P(PSTR("Accel offsets: %4.2f, %4.2f, %4.2f\n"),
                    (float)imu.ax(),
                    (float)imu.ay(),
                    (float)imu.az());
}
...
{
    hal.uart0->printf_P(PSTR("Gyro offsets: %4.2f, %4.2f, %4.2f\n"),
                    (float)imu.gx(),
                    (float)imu.gy(),
                    (float)imu.gz());
}
