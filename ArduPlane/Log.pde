...
print_log_menu(void)
{
    ...
    hal.uart0->printf_P(PSTR("logs enabled: "));

    if (0 == g.log_bitmask) {
        hal.uart0->printf_P(PSTR("none"));
    }else{
        // Macro to make the following code a bit easier on the eye.
        // Pass it the capitalised name of the log option, as defined
        // in defines.h but without the LOG_ prefix.  It will check for
        // the bit being set and print the name of the log option to suit.
 #define PLOG(_s)        if (g.log_bitmask & MASK_LOG_ ## _s) hal.uart0->printf_P(PSTR(" %S"), PSTR(# _s))
        PLOG(ATTITUDE_FAST);
        PLOG(ATTITUDE_MED);
        PLOG(GPS);
        PLOG(PM);
 ...
 #undef PLOG
    ...
    if (num_logs == 0) {
        ...
        for(int16_t i=num_logs; i>=1; i--) {
            ...
            hal.uart0->printf_P(PSTR("Log %d,    start %d,   end %d\n"), (int)temp, (int)log_start, (int)log_end);
...
dump_log(uint8_t argc, const Menu::arg *argv)
{
    ...
    if (dump_log == -2) {
        for(uint16_t count=1; count<=DataFlash.df_NumPages; count++) {
            ...
            hal.uart0->printf_P(PSTR("DF page, log file #, log page: %d,\t"), (int)count);
            hal.uart0->printf_P(PSTR("%d,\t"), (int)DataFlash.GetFileNumber());
            hal.uart0->printf_P(PSTR("%d\n"), (int)DataFlash.GetFilePage());
        }
        return(-1);
    } else if (dump_log <= 0) {
        hal.uart0->printf_P(PSTR("dumping all\n"));
        Log_Read(1, DataFlash.df_NumPages);
        return(-1);
    } else if ((argc != 2) || (dump_log <= (last_log_num - DataFlash.get_num_logs())) || (dump_log > last_log_num)) {
        hal.uart0->printf_P(PSTR("bad log number\n"));
        return(-1);
    }

    DataFlash.get_log_boundaries(dump_log, dump_log_start, dump_log_end);
    hal.uart0->printf_P(PSTR("Dumping Log %d,    start pg %d,   end pg %d\n"),
                    (int)dump_log,
                    (int)dump_log_start,
                    (int)dump_log_end);
...
select_logs(uint8_t argc, const Menu::arg *argv)
{
    ...
    if (argc != 2) {
        hal.uart0->printf_P(PSTR("missing log type\n"));
        return(-1);
    }
...
{
    hal.uart0->printf_P(PSTR("CURR: %d, %4.4f, %4.4f, %d\n"),
...
static void Log_Read_Nav_Tuning()
{
    ...
    hal.uart0->printf_P(PSTR("NTUN: %4.4f, %d, %4.4f, %4.4f, %4.4f, %4.4f, %4.4f,\n"),              // \n
                    d[0]/100.0,
                    (int)d[1],
                    ((uint16_t)d[2])/100.0,
                    ((uint16_t)d[3])/100.0,
...
static void Log_Read_Performance()
{
    ...
    hal.uart0->printf_P(PSTR("PM:"));
    pm_time = DataFlash.ReadLong();
    hal.uart0->print(pm_time);
    print_comma();
...
static void Log_Read_Cmd()
{
    ...
    for(int16_t i = 1; i < 4; i++) {
        ...
        hal.uart0->print(logvarb, DEC);
        print_comma();
    }
    for(int16_t i = 1; i < 4; i++) {
        logvarl = DataFlash.ReadLong();
        hal.uart0->print(logvarl, DEC);
        print_comma();
    }
    hal.uart0->println();
}
...
static void Log_Read_Startup()
{
    ...
    hal.uart0->printf_P(PSTR(" %d commands in memory\n"),(int)DataFlash.ReadByte());
}
...
static void Log_Read_Attitude()
{
    ...
    hal.uart0->printf_P(PSTR("ATT: %d, %d, %u\n"),
                    (int)d[0], (int)d[1],
                    (unsigned)d[2]);
}
...
static void Log_Read_GPS()
{
    ...
    hal.uart0->printf_P(PSTR("GPS: %ld, %d, %d, %4.7f, %4.7f, %d, %4.4f, %4.4f, %4.4f, %4.4f\n"),
...
static void Log_Read(int16_t start_page, int16_t end_page)
{
 ...
 #ifdef AIRFRAME_NAME
    hal.uart0->printf_P(PSTR((AIRFRAME_NAME)
 #endif
    hal.uart0->printf_P(PSTR("\n" THISFIRMWARE
                         "\nFree RAM: %u\n"),
    ...
    hal.uart0->printf_P(PSTR("Number of packets read: %d\n"), (int) packet_count);
}
                        ...
                        while (page < end_page && page != -1) {
                            ...
                            case 2:
                                if(data == LOG_ATTITUDE_MSG) {
                                    ...
                                    if(data == LOG_GPS_MSG) {
                                        ...
                                        hal.uart0->printf_P(PSTR("Error Reading Packet: %d\n"),packet_count);
                            ...
                            case 3:
                                if(data == END_BYTE) {
                                    ...
                                    hal.uart0->printf_P(PSTR("Error Reading END_BYTE: %d\n"),(int)data);
                                }
